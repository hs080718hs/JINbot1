import discord

client = discord.Client()


@client.event
async def on_ready():
    print("봇이 정상적으로 실행되었습니다.")
    game = discord.Game('출퇴근')
    await client.change_presence(status=discord.Status.online, activity=game)


@client.event
async def on_message(message):
    if message.content.startswith("/출근"):
        try:
            # 메시지 관리 권한 있을시 사용가능
            if message.author.guild_permissions.manage_messages:
                author = message.guild.get_member(int(message.author.id))
                embed = discord.Embed(color=0x80E12A)
                channel = 783992557201391617
                embed.set_author(name=author, icon_url=message.author.avatar_url)
                embed.add_field(name='관리자 출근 알림', value='모든문의 주세요!')
                embed.set_image(url="https://cdn.discordapp.com/attachments/782904776706490408/783992375773102090/IMG_20201105_004736-1.jpg")
                await client.get_channel(int(channel)).send(embed=embed)
        except:
            pass

    if message.content.startswith("/퇴근"):
        try:
            if message.author.guild_permissions.manage_messages:
                author = message.guild.get_member(int(message.author.id))
                embed = discord.Embed(color=0xFF0000)
                channel = 783992626562465792
                embed.set_author(name=author, icon_url=message.author.avatar_url)
                embed.add_field(name='관리자 퇴근 알림', value='문의들 못받습니다.')
                embed.set_image(url="https://cdn.discordapp.com/attachments/782904776706490408/783992375773102090/IMG_20201105_004736-1.jpg")
                await client.get_channel(int(channel)).send(embed=embed)
        except:
            pass

client.run('NzgxNzI3NjA0MTQyMTEyNzY5.X8B2kA.m6z1dZABxy12VHBBjDLwVnYGLpU')
